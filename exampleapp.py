...
def fbapi_app_token(id,old_token):
	token=fbapi_get_string(path="/oauth/access_token",params={'grant_type':'client_credentials','client_id':id,'client_secret':app.config['FB_APP_SECRET'],'redirect_url':get_home()})
	return token
...
def suggestion_new():
	if request.method=="GET":
	   return render_template('suggestion_new.html')
		...
		me=fb_call('me',args={'access_token': access_token})
		app_access_token=fbapi_app_token(FB_APP_ID)#,access_token)
